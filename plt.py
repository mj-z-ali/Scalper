...
def plot_resistance(bar_df_for_day: pd.DataFrame, resistance_index: np.array, resistance_level: float, date: str) -> None:
    ...
    ax.set_title(f'Bars with Resistance Line {resistance_level} for {date}')
    ax.set_xlabel('Time')
    ax.set_ylabel('Price')
...
def plot_resistances(bar_df_for_day: pd.DataFrame, resistance_indices: np.array, resistance_levels: np.array) -> None:
    ...
    list(map(lambda params: plot_res_func(resistance_index=params[0], resistance_level=params[1]), zip(resistance_indices, resistance_levels)))
